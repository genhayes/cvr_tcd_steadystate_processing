# G. Hayes 2023
# This file contains the functions used to process the data for the steady-state cerebrovascular reactivity (CVR) analysis in:
...
def get_average_heart_rate(tcd_data, sample_rate):
    '''hr_avg, f_oneside, X, n_oneside = get_average_heart_rate(tcd_data, sample_rate)
    where tcd_data is the tcd data to be analyzed (pandas Dataframe), and sample_rate is the sample rate of the data (Hz).'''
    X = np.fft.fft(tcd_data)
    N = len(X)
    n = np.arange(N)
    T = N/sample_rate
    freq = n/T
    ...
    # Get the one side frequency
    f_oneside = freq[:n_oneside]
    # Find index of X closest to a value in Hz
    idx = (np.abs(f_oneside - 0.66)).argmin()

    #find the peaks of in the tcd data to estimate the heart rate
    peaks_inds, peaks_vals = find_peaks(tcd_data, height=0, prominence=15, distance=0.5*sample_rate)
    # Shift CO2peaks_inds by the starting index of df_baseline1
    peaks_inds = peaks_inds + tcd_data.index[0]

    # calculate the heart rate
    hr_avg = (len(peaks_inds)/((tcd_data.index[-1]-tcd_data.index[0])/sample_rate))                

    #plot the peaks
    plt.plot(tcd_data)
    plt.plot(peaks_inds, peaks_vals['peak_heights'], 'x')
    plt.show()

    return hr_avg, f_oneside, X, n_oneside
...
def get_cvr(filepath, TF_to_plot, save_fig, save_csv, csv_filename, save_log, log_filename, comment_start_baseline1, comment_end_baseline1, comment_start_5co2, comment_end_5co2, comment_start_baseline2, comment_end_baseline2, TCD_calibration_factor, TCD_threshold, CO2prominence_base, O2prominence_base, CO2prominence_5co2, O2prominence_5co2, P_conversion_perc2mmHg, window_length_baseline1, window_length_5co2, window_length_baseline2):
    ...
    co2_data = df_5co2.iloc[:,1].astype(float)
    ...
    # save a value of the breathing rate during the baseline period and the 5% CO2 period
    br_avg_base = br_avg
    print('Average breathing rate during baseline:', 60*br_avg_base, 'breaths/min')
    br_avg_5co2, _, _, _ = get_average_breathing_rate(co2_data, sample_rate)
    print('Average breathing rate during 5% CO2:', 60*br_avg_5co2, 'breaths/min')
    ...
    hr_avg_base, _, _, _ = get_average_heart_rate(TCD_baseline1_th, sample_rate)
    hr_avg_5co2, _, _, _ = get_average_heart_rate(TCD_5co2_th, sample_rate)
    print('Average heart rate during baseline:', 60*hr_avg_base, 'beats/min')
    print('Average heart rate during 5% CO2:', 60*hr_avg_5co2, 'beats/min')
    ...
    if save_csv == True:
        ...
        if os.path.exists(csv_filename):
            with open(csv_filename, 'a', newline='') as file:
                ...
                writer.writerow([filename, CO2prominence_base, O2prominence_base, CO2prominence_5co2, O2prominence_5co2, MCAvmean_base, MCAvmean_5co2, CO2_mean_base*P_conversion_perc2mmHg, CO2_mean_5co2*P_conversion_perc2mmHg, O2_mean_base*P_conversion_perc2mmHg, O2_mean_5co2*P_conversion_perc2mmHg, percent_cvr, cvr, cvr_si])
        else:
    ...
    if save_log == True:
        if os.path.exists(csv_summary_filename):
            with open(csv_summary_filename, 'a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([filename, len_base1, len_5co2, len_base2, comment_start_baseline1, comment_end_baseline1, comment_start_5co2, comment_end_5co2, comment_start_baseline2, comment_end_baseline2, P_conversion_perc2mmHg, br_avg_base*60, br_avg_5co2*60, hr_avg_base*60, hr_avg_5co2*60])
                
        else:
            # Write the header and the data
            with open(csv_summary_filename, 'w', newline='') as file:
                writer = csv.writer(file)
                writer.writerow(["Input filename", "Length of baseline1 (s)", "Length of 5 CO2 (s)", "Length of baseline2 (s)", "comment_start_baseline1", "comment_end_baseline1", "comment_start_5co2", "comment_end_5co2", "comment_start_baseline2", "comment_end_baseline2", "P_conversion_perc2mmHg", "breathing_rate_baseline_BPM", "breathing_rate_5CO2_BPM", "heart_rate_baseline_BPM", "heart_rate_5CO2_BPM"])
                writer.writerow([filename, len_base1, len_5co2, len_base2, comment_start_baseline1, comment_end_baseline1, comment_start_5co2, comment_end_5co2, comment_start_baseline2, comment_end_baseline2, P_conversion_perc2mmHg, br_avg_base*60, br_avg_5co2*60, hr_avg_base*60, hr_avg_5co2*60])
...
def plot_4channel_data(plot_title, filepath, TCD_th, CO2, CO2_peaks, O2, O2_valleys, PPG, save_fig, MCAvmean):
    ...
    ax1.legend(['MCAv','MCAvmean'], fontsize=10)
    ...
    # ax2.legend(['CO2','End-Tidal CO2'], fontsize=10)
    ax2.set_xlabel('')
    ...
    # ax3.legend(['O2','End-Tidal O2'], fontsize=10)
    ax3.set_xlabel('')
